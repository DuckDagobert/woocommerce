...
class WC_Admin_Setup_Wizard {
	...
	protected function display_recommended_item( $item_info ) {
		...
		// All items are checked by default, unless otherwise specified.
		$should_be_checked  = isset( $item_info[ 'is_checked' ] ) ? $item_info[ 'is_checked' ] : true;
		?>
		<li>
			<label class="recommended-item">
				<input
					...
					type="checkbox"
					name="<?php echo esc_attr_e( 'setup_' . $type ); ?>"
					value="yes"
	...
	public function wc_setup_recommended() {
		...
		<p><?php
			...
			if (
				...
				esc_html_e( 'Select from the list below to enable automated taxes, Google Analytics, and MailChimp’s best-in-class email services — and design your store with our official, free WooCommerce theme.' );
			else :
				esc_html_e( 'Enhance your store with these recommended features.', 'woocoomerce' );
			endif;
		?></p>
		<form method="post">
			<ul class="recommended-step">
				...
				if ( $this->should_show_theme() ) :
					...
					$this->display_recommended_item( array(
						...
						'description' => sprintf( __(
								'Design your store with deep WooCommerce integration. If toggled on, we’ll install Storefront, and your current theme %s will be deactivated.', 'woocommerce' ),
								$theme_name
						),
						'img_url'     => WC()->plugin_url() . '/assets/images/storefront-temp.png',
				...
				if ( $this->should_show_automated_tax() ) :
					$this->display_recommended_item( array(
						...
						'img_url'     => WC()->plugin_url() . '/assets/images/taxes-temp.png',
